...
- [x] **Task: Main Thread Budget Audit**
    - [x] Profile in Chrome DevTools
    - [x] IDB reads <3ms typical
    - [x] rAF paint callbacks <3ms typical
    - [x] No long tasks (>50ms)
    - [x] Deterministic 60fps pacing during progressive reveal
- [x] **Task: Type Integrity Check**
    - [x] `npm run typecheck` passes with zero errors
    - [x] No `any` / no unsafe casts in spectrogram path
    - [x] Runtime validation catches schema mismatches + corrupted tiles
- [x] **Task: Final Verification Against Acceptance Criteria**
    - [x] 5-minute track spectrogram computes **exactly once** unless invalidated
    - [x] cached reload first visible tile <100ms
    - [x] partial analyses resume correctly (missing tiles only)
    - [x] tile metadata deterministic + complete
    - [x] crossfade eliminates seams
    - [x] status pill correct
    - [x] grid renders before data arrives (no layout shift)
    - [x] invalidation triggers on fingerprint/schema/tileSpec changes
